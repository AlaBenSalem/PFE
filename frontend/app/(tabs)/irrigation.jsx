// app/(tabs)/irrigation.jsx — CORRIGÉ
// Fix 1 : handleFaitPress — validation volume et temps avant POST
// Fix 2 : avertissement si débit non configuré sur la culture
// Fix 3 : setIsCompleted(true) seulement après fetchHistory() résolu
// Fix 4 : affichage en m³ au lieu de mm
...
// ✅ FIX 4 : conversion mm → m³
// eauMm (mm) × surface (m²) / 1000 = volume (m³)
...
export default function IrrigationPage() {
  ...
  const fetchKcDynamique = async (culture) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const result = await res.json();
      // ✅ FIX : le backend retourne result.kc directement (pas dans result.data)
      // On supporte les deux formats pour compatibilité
  ...
  const checkDebitMissing = (culture) => {
    const debit = parseFloat(culture?.irrigation?.debit);
    setDebitMissing(!debit || debit <= 0);
  };
  ...
  const calculateNeeds = () => {
    ...
    try {
      ...
      // ✅ CORRECTION DÉBIT — FAO-56 §7 :
      // Débit total (L/h) = débitGoutteur × nbGoutteursParArbre × nbArbres
      // Si les champs détaillés sont disponibles → calcul précis
      // Sinon → fallback sur irrigation.debit (champ legacy)
      ...
      const eta           = 1 - perte;
      const eauMm         = etc / eta;  // dose brute (mm)
      const perteMm       = eauMm - etc;

      // ✅ CORRECTION TEMPS — FAO-56 §7.4 :
      // Débit appliqué au sol (mm/h) = debitLH (L/h) / surface (m²)
      // => 1 L/m² = 1 mm donc débitMmh = debitLH / surface
      const debitMmh      = debitLH / surface;
      // Temps (min) = eauMm / debitMmh × 60
      // Ex : eauMm=5.3mm, debitLH=1000L/h, surface=400m² → debitMmh=2.5mm/h → t=5.3/2.5×60=127min=2h07 ✓
      const tempsMinutes  = debitMmh > 0 ? Math.round((eauMm / debitMmh) * 60) : 0;
      const volumeLitres  = Math.round(eauMm * surface);  // litres totaux
      ...
      // ✅ Volume/hectare (m³/ha) — FAO-56 §7
      // 1 mm sur 1 ha = 10 m³ → volume/ha = eauMm × 10 / efficacité
      const surfaceHa = surface / 10000;
      const volumeM3Ha = surfaceHa > 0
        ? (parseFloat(eauM3) / surfaceHa).toFixed(1)
        : null;

      // ✅ RU/RFU — FAO-56 §3.1 :
      // Si θcc et θpf renseignés → RU = (θcc − θpf) × z × 1000 (mm)
      // Sinon → table FAO-56 par type de sol
      ...
      const z     = selectedCulture.profondeurRacinaire || Z_DEFAUT[typeCult] || 0.6;
      const pAdj  = Math.min(0.8, Math.max(0.1, (P_BASE[typeCult] || 0.5) + 0.04 * (5 - etc)));

      // Valeurs θcc/θpf standard FAO-56 par type de sol (m³/m³)
      const THETA_STD = {
        sableux:         { cc: 0.18, pf: 0.06 },
        limono_sableux:  { cc: 0.26, pf: 0.10 },
        limoneux:        { cc: 0.34, pf: 0.14 },
        argilo_limoneux: { cc: 0.40, pf: 0.20 },
        argileux:        { cc: 0.46, pf: 0.26 },
      };

      // RU calculé depuis θcc/θpf si disponibles (FAO-56 Eq. 3.1)
      ...
      // Valeurs affichées : mesurées si dispo, sinon standards FAO-56
      const thetaCcDisplay = thetaCc ?? stdTheta.cc;
      const thetaPfDisplay = thetaPf ?? stdTheta.pf;
      const thetaSource = (thetaCc != null && thetaPf != null) ? "mesure" : "FAO-56";
      let ru;
      let ruSource;
      if (thetaCc != null && thetaPf != null && thetaCc > thetaPf) {
        ru = parseFloat(((thetaCc - thetaPf) * z * 1000).toFixed(1));
        ruSource = `θcc−θpf`;
      } else {
        ru = parseFloat(((RU_MM_PAR_M[typeSol] || 120) * z).toFixed(1));
        ruSource = `table FAO-56`;
      }
      ...
      return {
        ...
        thetaCcDisplay, thetaPfDisplay, thetaSource,
  ...
  return (
    <SafeAreaView className="flex-1 bg-gray-100">
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 20, paddingBottom: 36, paddingTop: 12 }} showsVerticalScrollIndicator={false}>
          ...
          <View className="flex-row items-start gap-2 bg-amber-50 border border-amber-300 rounded-2xl px-3.5 py-3 mb-4">
            ...
            <View className="flex-1">
              ...
              <Text className="text-[11px] text-amber-600 mt-0.5">
                La culture "{selectedCulture.nom}" n'a pas de débit défini. Le calcul utilise 1000 L/h par défaut. Configurez le débit dans "Mes cultures" pour un résultat précis.
              </Text>
            </View>
          </View>
        )}
        ...
        >
          <View className="flex-1">
            ...
            {selectedCulture ? (
              <>
                <Text className="text-[20px] font-semibold text-gray-900">{selectedCulture.nom}</Text>
          ...
          <>
            ...
            {hasData && !isCompleted && (
              <View className="bg-white rounded-2xl p-4 mb-4 shadow-sm elevation-3 border-l-4 border-sky-500">
                <View className="flex-row items-center gap-2 mb-2">
                  ...
                  <Text className="text-[14px] font-bold text-sky-700 flex-1">{t("irrigation.faoAdvice") || "Conseil d'irrigation FAO-56"}</Text>
                ...
                <Text className="text-[13px] text-gray-700 leading-5 mb-3">
                  ...
                  <Text className="font-bold text-gray-900">{selectedCulture.nom} ({selectedCulture.variete})</Text>
                ...
                <View className="flex-row items-center bg-slate-50 rounded-xl p-2.5 mb-3.5">
                  {[
                    ...
                    { label: "Kc (FAO-56)", val: besoins.kc, unit: kcStade || moisActuel, color: "text-violet-600", loading: loadingKc },
                    ...
                    <React.Fragment key={item.label}>
                      <View className="flex-1 items-center">
                        ...
                        {item.loading ? <ActivityIndicator size="small" color="#7c3aed" style={{ marginVertical: 4 }} /> : (
                ...
                <View className="flex-row justify-between items-start mb-3">
                  <View className="flex-1">
                    <Text className="text-[13px] text-gray-500 mb-1">{t("irrigation.volumeToApply") || "Volume à apporter"}</Text>
                    ...
                    <Text className="text-[11px] text-gray-400 mt-1">
                      {t("irrigation.etcTheoretical") || "ETc théorique"}: {besoins.eauTheoriqueMm} mm · {t("irrigation.losses") || "pertes"}: {besoins.perteMm} mm ({besoins.pourcentagePerte}%)
                    </Text>
                    {besoins.litresParArbre && (
                      <Text className="text-[11px] text-green-600 mt-1">
                        ≈ {besoins.litresParArbre} {t("irrigation.litersPerTree") || "L/arbre"}
                        {besoins.volumeLitres ? ` · ${besoins.volumeLitres.toLocaleString("fr-FR")} L ${t("irrigation.onSurface") || "sur"} ${besoins.surface} m²` : ""}
                      </Text>
                    )}
                  </View>
                  <View className="items-end">
                    ...
                    <Text className="text-[14px] font-semibold text-green-600 mt-0.5">{besoins.eta}% {t("irrigation.effShort") || "eff."}</Text>
                    {besoins.debitGoutteur > 0 && (
                      <Text className="text-[10px] text-gray-400 mt-0.5">
                        {besoins.debitGoutteur}L/h × {besoins.nbGoutteursParArbre}
                      </Text>
                    )}
                  </View>
                </View>
                ...
                <View className="flex-row items-start gap-2 bg-violet-50 border border-violet-200 px-2.5 py-2 rounded-xl mb-2">
                  ...
                  <View className="flex-1">
                    <View className="flex-row flex-wrap items-center gap-x-3 gap-y-0.5">
                      ...
                      <View className={`rounded-full px-2 py-0.5 ${besoins.thetaSource === "mesure" ? "bg-green-100" : "bg-violet-100"}`}>
                        <Text className={`text-[10px] font-semibold ${besoins.thetaSource === "mesure" ? "text-green-700" : "text-violet-500"}`}>
                          {besoins.thetaSource === "mesure" ? "✓ Mesure" : "FAO-56 std"}
                        </Text>
                      </View>
                    </View>
                    <Text className="text-[11px] text-violet-500 mt-0.5">
                      RU {t("irrigation.from") || "depuis"} <Text className="font-semibold">{besoins.ruSource}</Text>
                      {" · FAO-56 §3.1"}
                    </Text>
                  </View>
                </View>

                <View className="flex-row items-start gap-2 bg-blue-50 border border-blue-300 p-2.5 rounded-xl mb-2">
                  <Ionicons name="time-outline" size={15} color="#2563eb" />
                  <Text className="flex-1 text-[13px] leading-5 text-blue-800">
                    {t("irrigation.openValve") || "Ouvrez la vanne pendant"}{" "}
                    <Text className="font-bold">{fmtTemps(besoins.temps)}</Text>{" "}
                    {t("irrigation.atFlow") || "à"} <Text className="font-bold">{besoins.debitM3h} m³/h</Text>{" "}
                    ({t("irrigation.efficiency") || "efficacité"} {besoins.eta}%).
                  </Text>
                </View>
                ...
                <View className="flex-row items-start gap-2 bg-violet-50 border border-violet-300 p-2.5 rounded-xl mb-2">
                  ...
                  <Text className="flex-1 text-[13px] leading-5 text-violet-700">
                    RU = {besoins.ru} mm · RFU = {besoins.rfu} mm (p={besoins.pAdj}, z={besoins.z} m). {t("irrigation.frequency") || "Fréquence"} :{" "}
                    <Text className="font-bold">{t("irrigation.everyDays") || "tous les"} {besoins.frequenceJours} {t("irrigation.days") || "jours"}</Text>. {t("irrigation.next") || "Prochaine"} :{" "}
            ...
            {isCompleted && selectedCulture && (
              <View className="bg-green-50 border-2 border-green-200 rounded-2xl p-6 mb-4 items-center">
                ...
                <Text className="text-[22px] font-bold text-gray-900 mt-1.5">{selectedCulture.nom}</Text>
                {/* ✅ FIX 4 : affichage en m³ */}
                ...
                <Text className="text-[15px] text-green-600 mt-1">{t("irrigation.onSurface") || "sur"} {selectedCulture.surface} m²</Text>
                {besoins.litresParArbre && <Text className="text-[15px] text-green-600 mt-1">≈ {besoins.litresParArbre} {t("irrigation.litersPerTree") || "L/arbre"}</Text>}
                <Text className="text-[15px] text-green-600 mt-1">{besoins.debitM3h} m³/h · {fmtTemps(besoins.temps)}</Text>
                <Text className="text-[13px] text-gray-500 mt-1 text-center">ETc = {besoins.etc} mm · ET₀ {besoins.et0} × Kc {besoins.kc} · η = {besoins.eta}%</Text>
              </View>
            )}
          ...
          <View className="bg-white rounded-2xl p-5 shadow-sm elevation-2 mb-4">
            {historyItems.length === 0 ? (
              ...
              historyItems.map((item, idx) => {
                // ✅ FIX 4 : affichage historique en m³
                ...
                return (
                  <View key={item._id}>
                    <View className="flex-row justify-between mb-1">
                      <Text className="text-[14px] font-semibold text-orange-500">{item.nom || item.cultureId?.nom}</Text>
      ...
      <Modal visible={cultureModalVisible} transparent animationType="slide" onRequestClose={() => setCultureModalVisible(false)}>
        <TouchableOpacity className="flex-1 bg-black/40" activeOpacity={1} onPress={() => setCultureModalVisible(false)}>
          <SafeAreaView edges={["bottom", "left", "right"]} className="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl">
            <View className="flex-row justify-between items-center px-5 py-4 border-b border-gray-100">
              <Text className="text-[17px] font-bold text-gray-900">{t("irrigation.chooseCrop") || "Choisir une culture"}</Text>
            ...
            {cultures.length === 0 ? (
              <View className="items-center py-10">
                ...
                <Text className="text-[15px] text-gray-400 italic">{t("irrigation.noCulture") || "Aucune culture disponible"}</Text>
              </View>
            ) : (
              <FlatList
                data={cultures}
                ...
                renderItem={({ item }) => {
                  ...
                  return (
                    <TouchableOpacity
                      className={`flex-row items-center mx-4 my-1 p-4 rounded-xl border ${isSelected ? "bg-green-50 border-green-200" : "bg-gray-50 border-gray-100"}`}
                      onPress={() => handleSelectCulture(item)}
                    >
                      <View className="flex-1">
                        <Text className={`text-[15px] font-semibold ${isSelected ? "text-green-700" : "text-gray-900"}`}>{item.nom}</Text>
                        ...
                        <View className="flex-row flex-wrap mt-1.5 gap-1.5">
                          ...
                          {item.nombreArbres && (
                            <View className="bg-white border border-gray-200 rounded-full px-2 py-0.5">
                              <Text className="text-[11px] text-gray-600">{item.nombreArbres} {t("irrigation.trees") || "arbres"}</Text>
                            </View>
                          )}
                          <View className="bg-white border border-gray-200 rounded-full px-2 py-0.5">
                          ...
                          {itemDebitMissing && (
                            <View className="bg-amber-50 border border-amber-200 rounded-full px-2 py-0.5">
                              <Text className="text-[11px] font-semibold text-amber-600">⚠ {t("irrigation.flowMissing") || "Débit non défini"}</Text>
                            </View>
                          )}
                        </View>
                      </View>
